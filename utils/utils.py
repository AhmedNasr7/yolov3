...
def xywh2xyxy(x):
    ...
    y[:, 0] = x[:, 0] - x[:, 3] / 2         #leftX
    y[:, 1] = x[:, 1] - x[:, 2] / 2         #topY
    y[:, 2] = x[:, 0] + x[:, 3] / 2         #rightX
    y[:, 3] = x[:, 1] + x[:, 2] / 2         #rightY
    return y
...
def kmean_anchors(path='../coco/train2017.txt', n=9, img_size=(320, 640)):
    ...
    # Evolve
    wh = torch.Tensor(wh)
    f, sh, ng, mp, s = fitness(thr, wh, k), k.shape, 1000, 0.1, 0.3  # fitness, generations, mutation probability, sigma
    for _ in tqdm(range(ng), desc='Evolving anchors'):
        v = np.ones(sh)
        while (v == 1).all():  # mutate until a change occurs (prevent duplicates)
            v = ((np.random.random(sh) < mp) * np.random.randn(*sh) * s + 1) ** 2.0
